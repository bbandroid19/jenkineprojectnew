        	...
        	steps {
					...
					container_Up = false
                    			timeout(time: 1, unit: "HOURS") { 
                        			sh "curl -u admin:admin -X GET -H \"Accept: application/json\" http://104.248.169.167:9000/api/qualitygates/project_status?projectKey=com.mycompany:jenkineprojectnew > status.json"
    ...
    //     }
	 stage('Build') {
        ...
        stage('Deploy to Cloudhub'){
        	steps {
				 withCredentials([file(credentialsId: 'settings', variable: 'settings')]){
        	    	sh "mvn -f pom.xml -s $settings package deploy -DskipTests -Dusername=bejoy_njc -Dpassword=Mule1234 -DapplicationName=jenkineprojectnew -Denvironment=dev -DmuleDeploy -Dcloudhub.region=us-east-2 -Danypoint.platform.client_id=629b248d8a66690637c2a9d79070326781f282fe -Danypoint.platform.client_secret=sds -Dsecure.key=mule"
		...
		// 	steps{
		// 		sh "curl --location --request POST 'http://46.101.7.76:8087/logs'  --header 'Content-Type: application/json' --data-raw '{\"username\": \"bejoy_njc\",\"password\": \"Mule1234\",\"orgName\": \"NJC POC\",\"envName\": \"Sandbox\", \"appNames\": [\"jenkineprojectnew\"]}'"
		// 	}
		// }	
